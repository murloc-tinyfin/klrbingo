...
const specific_themed_songs = ["a theme from one of the 4 knights", "a final boss theme", "favourite theme playing", "a theme from the game in gameplay",
                               "a mini boss theme", "a remixed theme from previous games", "2 theme in a row from the same game"];
...
function generate_bingo_board() {
    ...
    let combined = [...shuffled_generic_songs, ...shuffled_specific_songs, ...shuffled_albums, ...shuffled_generic_gameplay,
    ...
    for (let i = 0; i < 5; i++) {
        tableHTML += '<tr>';
        for (let j = 0; j < 5; j++) {
            tableHTML += `<td style="padding: 5px;">${combined[i * 5 + j]}</td>`;
        }
    tableHTML += '</tr>';
    }
    tableHTML += '</table>';
